...
class Meal:
    ...
    def save(self):
        ...
        try:
            ...
            db.collection('users').document(self.user_id).collection(
                'meals').document(date_str).collection('entries').add(meal_data)
            return True
    ...
    def get_daily_meals(user_id, date_str):
        ...
        try:
            ...
            docs = db.collection('users').document(user_id).collection(
                'meals').document(date_str).collection('entries').stream()
    ...
    def delete_meal(user_id, date_str, meal_id):
        ...
        db = get_firestore()
        try:
            db.collection('users').document(user_id).collection('meals').document(
                date_str).collection('entries').document(meal_id).delete()
            return True
        except Exception as e:
